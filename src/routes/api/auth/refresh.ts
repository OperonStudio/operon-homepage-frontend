...
async function proxyToBackend(request: Request, path: string): Promise<Response> {
  ...
  // Forward cookie so the HttpOnly refresh token reaches the backend
...
export const ServerRoute = createServerFileRoute("/api/auth/refresh")({
  methods: {
    POST: async ({ request }) => proxyToBackend(request, "/api/auth/refresh"),
    OPTIONS: async ({ request }) => proxyToBackend(request, "/api/auth/refresh"),
  },
});
